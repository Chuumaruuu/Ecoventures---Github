...
public class ProductSpawner : MonoBehaviour
{
    ...
    public void SpawnProducts()
    {
        ...
        for (int i = 0; i < spawnCount; i++)
        {
            ...
            if (_inventoryData._finalProducts.Contains(itemData))
            {
                Debug.Log("Item available in inventory: " + itemData.name);
                int count = _inventoryData._finalProducts.Where(item => item == itemData).Count();
                Debug.Log("Count of item in inventory: " + count);
            }else
            {
                Debug.Log("Item not available in inventory: " + itemData.name);
            }
